...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        listOfNextStateAndProbPairs = self.mdp.getTransitionStatesAndProbs(state, action)
        value = 0
        for pair in listOfNextStateAndProbPairs:
          (nextState, prob) = pair
    ...
    def computeActionFromValues(self, state):
        ...
        qValue = self.computeQValueFromValues(state, actions[0])
        bestAct = (actions[0], qValue)
        for action in actions:
          qValue = self.computeQValueFromValues(state, action)
          if qValue > bestAct[1]:
            bestAct = (action, qValue)
        return bestAct[0]
        util.raiseNotDefined()
