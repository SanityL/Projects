...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("It's an ill-formed list.")
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Too few or too many arguments")
        else:
            counter = 0
            while counter < len(formals):
                frame.bindings[formals[counter]] = vals[counter]
                counter += 1
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            name = target[0]
            variables = target.second
            lambda_body = Pair(variables, vals.second)
            env.define(name, do_lambda_form(lambda_body, env))
            return name
        else:
            raise SchemeError
    else:
...
def do_let_form(vals, env):
    ...
    for variableItem, valueItem in enumerate(bindings):
        names = Pair(valueItem[0], names)
        values = Pair(scheme_eval(valueItem[1], env), values)
...
def do_if_form(vals, env):
    ...
    if scheme_true(scheme_eval(vals.first, env)):
        return vals[1]
    elif len(vals)>2: #which means the predicate is not true
        return vals[2]
    return okay
...
def do_and_form(vals, env):
    ...
    for i in range(len(vals)-1):
        if not scheme_true(scheme_eval(vals[i], env)):
            return False
        curr = vals[i+1]
    return quote(scheme_eval(curr, env))
...
def do_or_form(vals, env):
    ...
    curr = vals[0] if len(vals)>0 else False
    for i in range(len(vals)-1):
...
def do_begin_form(vals, env):
    ...
    if length == 1:
        ...
        for item in range(length-1):
            scheme_eval(vals[item], env)
        return vals[length - 1]
    #check if it is last item
    #evaluate items one by one
...
def check_formals(formals):
    ...
    """
    if scheme_listp(formals) is False:
        raise SchemeError("not well-formed list")
    else:
        formal_list = []
        for el in formals:
            if scheme_symbolp(el) is False:
                raise SchemeError("Invalid Symbol")                
            if el in formal_list:        
                raise SchemeError("Duplicate parameters")
            else:
                formal_list.append(el)
