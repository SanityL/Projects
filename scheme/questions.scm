...
(define (list-partitions total max-pieces max-value)
  ...
  (define (make-partition total max-pieces max-value partition_list)

    (cond 
      ((zero? total) (list partition_list))
      ((< total 0) nil)
      ((zero? max-value) nil)
      (else (append (make-partition (- total max-value) max-pieces max-value (append (list max-value) partition_list))
        (make-partition total max-pieces (- max-value 1) partition_list)))))

   (filter (lambda (x) (<= (length x) max-pieces)) (make-partition total max-pieces max-value '())))
...
(define (flatten list_of_trees)
  (cond
    ((null? list_of_trees) nil)
    ((list? (car list_of_trees)) (append (flatten (car list_of_trees)) (flatten (cdr list_of_trees))))
    (else (cons (car list_of_trees) (flatten (cdr list_of_trees)))))) ;If the first element is a number
