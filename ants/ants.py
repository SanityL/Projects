...
# Name: Shu-Hung Lin (Jacob)
# Email: jacoblin@berkeley.edu
...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant!=None and self.ant.container and self.ant.can_contain:
                self.ant.contain_ant(insect)
                return
            if self.ant!=None and insect.container and insect.can_contain:
...
class Ant(Insect):
    ...
    def double(self):
        "QueenAnt doubles the damage of the ants in the tunnel by calling this method."
        if not self.doubled:
            self.doubled = True
    ...
    def nearest_bee(self, hive):
        ...
        while cur_place != hive:
            if cur_place.bees == []:
                ...
                cur_range += 1
            elif cur_range >= self.min_range and cur_range <= self.max_range:
...
class BodyguardAnt(Ant):
    ...
    def double(self):
        Ant.double(self)
        if self.ant !=None and not type(self.ant) == QueenAnt:
            Ant.double(self.ant)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        QueenAnt.num += 1
        self.impostor = QueenAnt.num > 1

    def double(self):
        """Queen's damage is not doubled."""
        return
    ...
    def action(self, colony):
        ...
        front_place, back_place = self.place, self.place
        while front_place!=None:
            if front_place.ant:
                front_place.ant.double()
            front_place = front_place.entrance
        while back_place!=None:
            if back_place.ant:
                back_place.ant.double()
...
def apply_effect(effect, bee, duration):
    ...
    def make_effect(colony):
        """Change the action of the bee to the effected one for a duration."""
        nonlocal duration
        if duration==0:
