        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for oldPos in self.legalPositions:
                if util.manhattanDistance(p, oldPos)<=1:
                    newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
                    probSum += self.beliefs[oldPos] * newPosDist[p]
            allPossible[p] = probSum
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None:
            ...
            for particle in self.particles:
                ...
                weightedParticleDistri[particle] += emissionModel[trueDistance]
            # second special case
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            beliefs.append(util.sample(newPosDist))   
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefDistri = util.Counter()
        for particle in self.particles:
            beliefDistri[particle] += 1
        beliefDistri.normalize()
        return beliefDistri
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = [particleList[i % len(particleList)] for i in xrange(self.numParticles)]
