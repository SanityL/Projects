...
def roll_dice(num_rolls, dice=six_sided):
    ...
    n = 1
    sum_of_outcome = 0
    there_is_a_1 = False # becomes True if there is any 1
    while n <= num_rolls:
        outcome = dice()
        if outcome == 1: # when the dice rolls any 1
            there_is_a_1 = True # becomes true
        else:
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        return max(opponent_score%10, opponent_score//10) + 1 # Free bacon's rule
    else:
...
def make_averaged(fn, num_samples=1000):
    ...
    """
    def average(*args):
        """Average the sum of the values."""
        n = 1
        sum_of_values = 0 # Record the sum of the values returned by FN
        while n <= num_samples: # Call FN NUM_SAMPLES times and obtain the sum of the returned values
            sum_of_values = fn(*args) + sum_of_values
            n += 1
        return sum_of_values/num_samples # Return the average of the sum of the returned values
    return average
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    """
    n = 1
    highest_average_turn_score, max_scoring_roll = 0, 0
    make_avg = make_averaged(roll_dice)
    while n <= 10:
        avg = make_avg(n, dice)
        print(n, "dice scores", avg, "on average")
        if avg > highest_average_turn_score:
            highest_average_turn_score = avg
            max_scoring_roll = n # Starting from roll 1, update the number of roll every time another roll is greater
        n += 1
...
def bacon_strategy(score, opponent_score):
    ...
    0
    """
    units_digit = opponent_score%10 # UNITS_DIGIT of the opponent's score = 7 if 87 is given
    tens_digit = opponent_score//10 # TENS_DIGIT of the opponent's score = 8 if 87 is given
    if units_digit >= (BACON_MARGIN-1) or tens_digit >= (BACON_MARGIN-1):
        return 0
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    max_op_digit = max(opponent_score%10, opponent_score//10) #To get the largest digit of the opponent's score
    if score+max_op_digit+1==opponent_score/2: #beneficial swap
        return 0
    elif score+max_op_digit+1==opponent_score*2: #harmful swap
        return BASELINE_NUM_ROLLS
    elif max_op_digit>=BACON_MARGIN-1: #at least bacon margin points earned by rolling 0
        return 0
    return BASELINE_NUM_ROLLS

def final_strategy(score, opponent_score):
    """Increase the rate of winning by leaving the opponent with four-sided dice more often,
       then by taking fewer risks if wining or taking more risks if losing, and finally by 
       varing rolls based on whether you will be rolling four-sided or six- sided dice.
    """
    max_op_digit = max(opponent_score%10, opponent_score//10) #To get the largest digit of the opponent's score
    if score > opponent_score: #winning
        BASELINE_NUM_ROLLS = 4
    elif score < opponent_score: #losing
        if (score+opponent_score)%7==0: #check if starting with the four-sided dice
            BASELINE_NUM_ROLLS = 4
        else:
    ...
    if score+max_op_digit+1==opponent_score/2: #beneficial swap
            ...
            return 0
    elif (score+opponent_score+max_op_digit+1)%7==0: #leave the opponent with the four-sided dice to roll
            return 0
    return BASELINE_NUM_ROLLS
