...
def find_centroid(polygon):
    ...
    centroid_y = 0
    i = 0
    def first_multiplier_of_c_x(a):
        """Returns the first multiplier of Cx, (Xi + Xi+1)."""
        return latitude(polygon[a])+latitude(polygon[a+1])
    def first_multiplier_of_c_y(b):
        """Returns the first multiplier of Cy, (Yi + Yi+1)."""
        return longitude(polygon[b])+longitude(polygon[b+1])
    def area_formula(c):
        """Returns the argument of the area formula, (XiYi+1 - Xi+1Yi)."""
        return latitude(polygon[c])*longitude(polygon[c+1])-latitude(polygon[c+1])*longitude(polygon[c])

    while i < len(polygon)-1:
        area += (1/2)*area_formula(i)
        centroid_x += first_multiplier_of_c_x(i)*area_formula(i) #The second multiplier equals area formula
        centroid_y += first_multiplier_of_c_y(i)*area_formula(i)
        i += 1
    if area == 0: #Since AREA/6 = AREA if AREA = 0
...
def find_state_center(polygons):
    ...
    while i < len(polygons):
        sum_of_centroid_x = sum_of_centroid_x + (find_centroid(polygons[i])[0])*(find_centroid(polygons[i])[2])
        sum_of_centroid_y = sum_of_centroid_y + (find_centroid(polygons[i])[1])*(find_centroid(polygons[i])[2])
        sum_area = sum_area + find_centroid(polygons[i])[2]
        i += 1
    return make_position(sum_of_centroid_x/sum_area, sum_of_centroid_y/sum_area)
...
def group_tweets_by_state(tweets):
    ...
    tweets_by_state = {}
    state_ctrs = {}
    for state in us_states: #To make the calculation faster
        state_ctrs[find_state_center(us_states[state])] = state
    for tweet in tweets:
        min_dist = 1000 #Initialize the minimum distance
        closest_state = '' #Initialize the closest state
        for ctr in state_ctrs:
            current_dist = geo_distance(tweet_location(tweet), ctr)
            if current_dist < min_dist:
                min_dist = current_dist
                closest_state = state_ctrs[ctr]
        if closest_state in tweets_by_state:
            tweets_by_state[closest_state].append(tweet)
        else:
            tweets_by_state[closest_state] = [tweet] #Create the key and add tweet as a list to the key
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        sum_sentiment = 0
        i = 0 #Check # of sentiments if the tweet has at least a sentiment value
        for tweet in tweets_by_state[state]:
            if has_sentiment(analyze_tweet_sentiment(tweet)):
                ...
                i += 1
        if i != 0:
            averaged_state_sentiments[state] = sum_sentiment/i
