...
def joinFactors(factors):
    ...
    unconditionedVariables = list(first.unconditionedVariables()) + list(second.unconditionedVariables())
    conditionedVariables = list(first.conditionedVariables()) + list(second.conditionedVariables())
    conditionedVar = []
    ...
    newFactor = Factor(unconditionedVariables, conditionedVar, first.variableDomainsDict())
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for each in newFactor.getAllPossibleAssignmentDicts():
            probability = 0
            for originalEach in factorAssignments:
                if set(each.items()).issubset(set(originalEach.items())):
                    probability += factor.getProbability(originalEach)
            newFactor.setProbability(each, probability)
